...
impl Uri {
    ...
    #[doc(hidden)]
    pub unsafe fn __internal_from_utf8_unchecked(slice: Bytes) -> Result<Uri, UriError> {
        Uri::new(ByteStr::from_utf8_unchecked(slice))
    }

    #[doc(hidden)]
    pub fn __internal_scheme_and_authority(&self) -> Option<Uri> {
        let uri = self;
    ...
    #[doc(hidden)]
    pub fn __internal_origin_form(&self) -> Uri {
        let uri = self;
...
fn test_uri_to_origin_form() {
    ...
    for case in cases {
        ...
        assert_eq!(uri.__internal_origin_form(), case.1); //, "{:?}", case);
    }
}
